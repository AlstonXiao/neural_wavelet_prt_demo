...
namespace nert_renderer {
  struct GLFWindow {
    GLFWindow(const std::string &title, const gdt::vec2i size);
    ~GLFWindow();
    ...
    virtual void key(int key, int mods, int action)
    {}
    
    /*! callback that window got resized */
    virtual void mouseMotion(const gdt::vec2i &newPos)
    {}
    ...
    inline gdt::vec2i getMousePos() const
    {
      double x,y;
      glfwGetCursorPos(handle,&x,&y);
      return gdt::vec2i((int)x, (int)y);
    }
  ...
  struct CameraFrame {
    ...
    void setOrientation(/* camera origin    : */const gdt::vec3f &origin,
                        /* point of interest: */const gdt::vec3f &interest,
                        /* up-vector        : */const gdt::vec3f &up)
    {
      position = origin;
      upVector = up;
      frame.vz
    ...
    {
      return gdt::max(gdt::max(computeStableEpsilon(v.x),
  ...
  struct CameraFrameManip {
    ...
    virtual void keyUpdate(float deltaTime) {
        CameraFrame& fc = *cameraFrame;
        if (hold_down['w']) {
            fc.position += normalize(fc.getPOI() - fc.position) * .2f * deltaTime;
            fc.modified = true;
        } if (hold_down['a']) {
            fc.position += fc.frame.vx * -.2f * deltaTime;
            fc.modified = true;
        } if (hold_down['s']) {
            fc.position += normalize(fc.getPOI() - fc.position) * -.2f * deltaTime;
            fc.modified = true;
  ...
  struct GLFCameraWindow : public GLFWindow {
    ...
    /*! callback that window got resized */
    virtual void mouseMotion(const gdt::vec2i &newPos) override
    {
        gdt::vec2i windowSize;
